...
export class ExpressLoader extends AbstractLoader {
    private static reorderRoutes(app) {
        let jsonParser;
        ...
        if (app && app._router && app._router.stack) {
            ...
            const adminIndex = app._router.stack.findIndex((layer: { name: string }) => layer.name === "admin");
            if (adminIndex >= 0) {
            ...
            const corsIndex = app._router.stack.findIndex((layer: { name: string }) => layer.name === "corsMiddleware");
    ...
    public register(admin: AdminBro, httpAdapter: AbstractHttpAdapter, options: AdminModuleOptions) {
        ...
        ExpressLoader.reorderRoutes(app);
    }
}
